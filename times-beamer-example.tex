...
\begin{eqnarray*}
\vec{x} \cdot \vec{y} & = & \vec{y} \cdot \vec{x} \qquad \mbox{(commutative)} \\
\vec{x} \cdot (\vec{y} + \vec{z}) & = & \vec{x} \cdot \vec{y} + \vec{x} \cdot \vec{z} \qquad \mbox{(distributive)}\\
(k\vec{x}) \cdot \vec{y} & = & \vec{x} \cdot (k\vec{y}) = k(\vec{x} \cdot \vec{y}) \qquad \mbox{where $k$ is a scalar} \\
...
\draw[thick,blue,->] (0,0) -- (2,3);
\draw (2,3) node[left, font=\small] {$\vec{y}$};

\draw[thick, ->] (2,0) -- (2,3);
\draw (2,3) node[below right, font=\small] {$\widehat{y}_{\bot}$};

\draw[thick,->] (0,0) -- (2,0);
\draw (2,0) node[below left, font=\small] {$\widehat{y}$};
...
Consider a data set in which we've measured variables ${X_1, X_2,\ldots,X_p}$, on a set of subjects (objects) $a_1,...,a_n$.
...
\item The above can be written as $|\vec{x}-\vec{1}\bar{x}|^2$ where $\vec{1} = [1,1,\ldots,1]'$

\item We are looking, therefore, for the scalar multiple, $\bar{x}$, of the unit vector that minimizes $|\vec{x}-\vec{1}\bar{x}|^2$
...
\item Recall that:
\begin{eqnarray}
P_{\vec{1}}(\vec{x}) & = & \vec{1}\bar{x} \ \mbox{for some}\ \bar{x} \\
%
(\vec{x} - P_{\vec{1}}(\vec{x})) \cdot \vec{1} & = & 0 
\end{eqnarray}

\item Substituting (1) into (2):
\begin{eqnarray}
(\vec{x} - \vec{1}\bar{x}) \cdot \vec{1} & = & 0 \\
%
\vec{x} \cdot \vec{1} & = & \bar{x} (\vec{1} \cdot \vec{1})
\end{eqnarray}
...
\draw[thick,->] (0,0) -- (6,0);
\draw (6,0) node[above,font=\tiny] {$\vec{1} $};

\draw[thick,red,->] (0,0) -- (4,0);
\draw (4,0) node[below,font=\footnotesize] {$P_{\vec{1}}(\vec{x}) = [\bar{x}, \bar{x},\ldots,\bar{x}]$};

\draw[thick,->] (0,0) -- (4,3);
\draw (4,3) node[above] {$\vec{x}$};

\draw[thick,dashed,blue,->] (4,0) -- (4,3);
\draw (4,3) node[below right,font=\footnotesize] {$\vec{e} = \vec{x} - P_{\vec{1}}(\vec{x}) = [x_1-\bar{x}, x_2 - \bar{x},\ldots,x_n - \bar{x}]$};
...
\draw[thick,red,->] (0,0) -- (4,0);
\draw (4,0) node[below,font=\footnotesize] {$P_{\vec{\mathbf{1}}}(\vec{x})$};
...
%===========================================================
\begin{frame}
  \frametitle{Bivariate Regression as Projection II}
...
We showed on the previous page that 
\[
\vec{\widehat{y}} = b\vec{x} \qquad \mbox{and} \qquad b= \frac{\vec{x} \cdot \vec{y}}{|\vec{x}|^2}.
\] 

We can rewrite this in the following ways:
\begin{eqnarray*}
b &=& |x||y| \cos \theta / |x|^2 \\
  &=& \cos \theta (|y|/|x|) \\
  &=& r_{XY}(|y|/|x|)
...
\begin{itemize}
  \item Wickens, Chapters 1-3
  \item Kampstra, P. 2008. Beanplot: A boxplot alternative for visual comparison of distributions. \textit{J. of Statistical Software}, 28. 
  \item Paradis, E. 2005. R for beginners. Chapter 4. 
\end{itemize}
